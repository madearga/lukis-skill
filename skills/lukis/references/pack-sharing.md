...
spec), `packs install <name>` (→ `~/.config/lukis/characters/<name>/`), or
...
2. `packs install <name>`. It refuses to overwrite an existing local pack:
   ...
   - To install the whole community catalog for local browsing or Grok Bot
     ...
     `python3 "$SKILL_DIR/scripts/lukis.py" packs install --all`. Quoted
...
3. Offer to make it the default
   (`python3 "$SKILL_DIR/scripts/lukis.py" init --no-key --character <localname>`)
...
```bash
SKILL_DIR="<path to this skill>";
python3 "$SKILL_DIR/scripts/lukis.py" packs update <name>
```
...
Prerequisites: the pack exists locally (`~/.config/lukis/characters/<name>/`),
