...
The user may prompt casually ("cutout of Ciko waving", "yoke sitting on a
...
in lukis's editorial sense — if that is what the ask needs, reroute to editorial.
...
python3 "$SKILL_DIR/scripts/lukis.py" generate --prompt-file /tmp/cutout.txt --ref "$REF" --aspect 1:1 --cutout --image-config '{"aspect_ratio":"1:1"}' --out /tmp/lukis-cutout-ciko-wave.png
```

### Chroma compatibility screen

An optional **`Cutout chroma: green`** or **`Cutout chroma: magenta`** line in
`character.md` selects the pack's compatibility screen (Ciko: magenta in
...
- Soft blob / droplet (Ciko) — jelly squash of the body; tip rides the squash;
  feet planted.
...
Do **not** use Plukisw `save(..., optimize=True, disposal=2)` for this path; it
...
- A frame has nearly zero transparent pixels, or the GIF flashes black →
  re-encode with the ffmpeg palettegen path above; never use Plukisw
