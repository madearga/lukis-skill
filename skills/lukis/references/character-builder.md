...
- **What is it?** An object or creature from the user's domain, product, or
  ...
  simple silhouette.
- **What look?** The pack's one style: sablon (house default) or another from
  the look library — riso, blueprint, woodcut, pixel, clay, manila, chalk,
...
Style: **{look name — sablon if unset}**
...
LINE LANGUAGE: ONE thick, even-width outline with squared terminals (a
solid marker pulled through a stencil), flat spot-color fills inside,
nothing thin or scratchy.

STYLE: hand-pulled screen print — flat opaque spot-color inks on heavy
cotton paper, one flat tone per shape, no halftone, no gradients; faint
hand-cut stencil wobble on edges; heavy paper tooth.

PALETTE: paper warm cream #f6ecd9. Ink warm near-black #1d1a15. Accent
chili red #d4492e ONLY on {the accent part}.
```
...
non-sablon look, substitute the style file's LINE LANGUAGE and STYLE blocks and
...
isn't publishing, and avoid the reserved names `ciko`, `lukis`, and the look
names (`sablon`, `riso`, `blueprint`, `woodcut`, `pixel`, `clay`, `manila`, `chalk`,
`phosphor`, `enamel`, `gouache`, `felt`, `diorama`, `sketchbook`, `bricks`, `fizz`, `bloom`, `snes`). With a winner chosen:

```bash
PACK="${XDG_CONFIG_HOME:-$HOME/.config}/lukis/characters/<name>"
mkdir -p "$PACK"
cp <chosen-render>.png "$PACK/reference.png"
# write the filled step-3 template to "$PACK/character.md"
```

Confirm with `python3 "$SKILL_DIR/scripts/lukis.py" doctor` — it lists the
pack. Then ask whether this should become the **default character**; if yes,
set it (non-secret, so you may run it):

```bash
SKILL_DIR="<path to this skill>";
python3 "$SKILL_DIR/scripts/lukis.py" init --no-key --character <name>
```
...
SKILL_DIR="<path to this skill>";
PACK="${XDG_CONFIG_HOME:-$HOME/.config}/lukis/characters/<name>";
python3 "$SKILL_DIR/scripts/lukis.py" generate --prompt-file /tmp/<name>-cutout-proof.txt --ref "$PACK/reference.png" --aspect 1:1 --cutout --chroma <green-or-magenta> --out /tmp/<name>-cutout-proof.png
```
...
1. Name it `<name>-<style>` (e.g. `ciko-woodcut`). Identity is unchanged:
