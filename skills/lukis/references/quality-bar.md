...
The checks below assume sablon; when the active character's style isn't sablon,
swap the sablon-specific checks (stencil edge, ink seams, squeegee) for the **QA deltas in
...
- Correct aspect ratio; the style's expected ground (sablon: heavy cotton
  paper with the stencil edge; riso: light paper with the risograph grain;
  other styles: per their QA deltas — e.g. blueprint's deep ground is correct).
...
- **Mascot is on-model**: matches the active character's locked design (the
  default Ciko's in `character.md`, or the custom pack's) — the locked face
