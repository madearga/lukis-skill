...
The print-shop look is the constant identity; the palette is a swappable
parameter. Every palette obeys the same color grammar defined by the active
style's file:
...
1. **Explicit request** — "use pasar", a custom palette by name, "make it emerald", a brand hex.
...
4. **House default** — `pasar`.

## Named presets

| preset | paper | structure ink | accent(s) | mascot body | notes |
|---|---|---|---|---|---|
| **`pasar`** (house default) | warm cream `#f6ecd9` | warm near-black `#1d1a15` | chili red `#d4492e` + leaf teal `#1a6e5f` | dark OK | bold market-banner warmth |
| `ink-punch` | warm white `#fffef7` | near-black `#111111` | fluoro pink `#ff3d9a` | dark OK | boldest, most electric |
...
`${XDG_CONFIG_HOME:-~/.config}/lukis/palettes.md` (the `doctor` subcommand
