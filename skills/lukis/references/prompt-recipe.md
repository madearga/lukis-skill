...
default Ciko, or the custom pack's `character.md`). End with concrete hex
values from `palettes.md`.

The template below is written for **sablon**. When the active character's pack
...
**Registration-locked silhouette** — cutouts must NOT carry the editorial
style's print texture along the outer contour; see the SILHOUETTE block
below. Do **not** add a `BACKGROUND:` or
...
LINE LANGUAGE: draw the mascot and any contact objects with ONE thick, even-width outline with squared terminals (a solid marker pulled through a stencil), flat spot-color fills inside, not thin scratchy sketch lines.

SILHOUETTE (cutout — registration-locked): ONE locked outer contour only. All inks aligned on the same edge — NO misregistration, NO ghost plate, NO second copy of the body outline, NO accent-colored halo or fringe tracing the silhouette, NO bare-paper seam along the outer edge. Accent ink ONLY on the designated accent part, never bleeding along the outer edge.

STYLE: HAND-PULLED SCREEN PRINT — flat opaque spot-color fills on the character and contact cluster only — NOT on the background; no halftone, no gradients, registration-locked single-plate silhouette.
...
For non-sablon looks, substitute LINE LANGUAGE, STYLE, and PALETTE from the active
style file as usual — keep the SILHOUETTE block, and drop any style texture
(halftone, offset, seams) from the outer edge in the style's STYLE line.
...
the model hand-letters it in the active style's lettering:
...
Pass the existing image back as a `--ref` to `lukis.py generate` (instead of, or
