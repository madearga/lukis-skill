...
- "surprise me", "surprise", "lukis surprise"
- "random", "random lukis", "give me something random"
- scoped variants: "surprise me with art quote", "random using ciko",
...
2. **Step 0 preflight** (`doctor`) — resolve hard blockers (including
   `backend: NEEDS CHOICE` when you will use `lukis.py generate`) **before** any
...
2. **Character** — phrases like `using ciko`, `with bray`, `as blip`.
...
character list, then **add shipped `ciko`** if it is not already present.
...
- **“Three new ones” / refresh** — this is a **full re-roll**, not a deeper
  ...
  1. **Character** — if the user named one (`using ciko`, `as bray`), keep
