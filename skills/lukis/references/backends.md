...
lukis has **three engine backends** plus one named **agent-side transport**. All
...
- **Grok Bot native** — when the agent is **Grok Bot** (Cursor's Grok Bot /
  ...
  tool directly with lukis's prompt and reference image. This is the same Grok
...
`lukis.py generate` refuses `grok-bot` because only the Grok Bot agent can call
...
- **Grok Bot** persists `backend: grok-bot` with
  ...
  call `lukis.py generate`; it builds the same lukis prompt and calls Grok Bot's
...
3. `codex features list` reports the `image_generation` row. Codex 0.144 folded
   ...
   was removed once the behavior went stable, so lukis no longer gates on it.)
...
`codex login` — that is the entire setup. lukis never touches the token.
...
Aspect has no size argument on the free tool either; lukis states the aspect
...
For `--cutout`, lukis appends a native transparent-PNG contract to the Codex
...
lukis invokes `codex exec` against the built-in tool, attaching the active
...
`$CODEX_HOME/generated_images/<session-id>/<image>.png` on its own — lukis
...
once the behavior went stable, so lukis no longer passes the flag.) Artifact
...
With no `--ref` and no
default character there is nothing to lock to, so lukis renders ref-less (a
...
step 4). lukis handles **no token**: it runs no OAuth, reads no
...
lukis treats that as a backend failure. Select OpenRouter directly, or explicitly
permit the paid retry with `--allow-paid-fallback` when a key is configured.

### Fallback behavior

When the Codex backend is unavailable or produces no valid fresh artifact, lukis
...
fallback lands on OpenRouter, lukis replaces its native-alpha contract with the
...
reference image (`assets/character-reference.webp` for Ciko, or the pack's
...
path is the `.path` equivalent for QA and delivery. `lukis.py generate` refuses
...
like Codex). Aspect is honored: lukis states it in the prompt and Grok's tool
...
key color — so chroma-keying fails (opaque corners, heavy fringe). lukis does
...
before enabling Grok. lukis invokes `grok -p` with `--always-approve --cwd
...
When the Grok CLI backend is unavailable or produces no retrievable image, lukis
