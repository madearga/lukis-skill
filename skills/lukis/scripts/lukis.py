...
def config_dir():
    ...
    return pathlib.Path(base) / "lukis"
...
def dump_config_yaml(cfg):
    ...
    out = [
        "# ~/.config/lukis/config.yaml — Lukis settings. All keys optional.",
        "# Set the API key once with: lukis.py init (stored here, file mode 600).",
        "",
        f"configVersion: {CONFIG_VERSION}   # schema marker; set by init — do not edit",
        "",
        f"apiKey: {val(cfg['apiKey'])}" if cfg.get("apiKey")
        else "# apiKey: sk-or-...           # set via: lukis.py init",
        ...
        else "# defaultPalette: signal     # preset or custom palette name; default: pasar",
...
def migration_message():
    ...
    return (
        "lukis config is out of date — it predates the image-backend choice, so "
        "no backend is selected.\n"
        "lukis now has image backends/transports. Pick one, then re-run:\n"
        ...
        f"  Grok Bot   — agent-side native image tool (not lukis.py generate):\n"
...
def aspect_to_image_config(aspect):
    """Map lukis --aspect hints to OpenRouter image_config.aspect_ratio."""
    if not aspect:
        return {}
    ...
    for ref in refs or []:
        ...
        if pack:
            ...
            if chroma:
                return chroma
    for ref in refs or []:
        if pathlib.Path(ref).expanduser().resolve() in bundled_ciko_ref_paths():
            return shipped_ciko_cutout_chroma() or "magenta"
...
def run_base():
    return pathlib.Path(os.environ.get("LUKIS_TMP") or "/tmp/lukis")
...
def codex_exec_generate(prompt, refs, out_path):
    ...
    BackendUnavailable. lukis handles no token; the only privileged action is
...
def cmd_generate(args):
    ...
    if backend is None:
        ...
        sys.exit(f"No image backend ready. Install + `codex login` (or `grok login`) "
                 ...
                 "Grok Bot image tool instead of `lukis.py generate`.")
    if backend == "grok-bot":
        sys.exit("backend grok-bot is agent-side: `lukis.py generate` cannot call "
                 "Grok Bot's native image tool. The Grok Bot agent should build "
                 "the lukis prompt and call its built-in Grok image tool with the "
                 "active character sheet as a reference. To use `lukis.py generate`, "
...
def _resolve_refs(refs, cfg):
    ...
    any finished-look style anchor lukis adds for set consistency); else falls
...
def _maybe_offer_codex(cfg):
    ...
    print("lukis can generate images through your Codex subscription — free, but it "
...
def _maybe_offer_grok(cfg):
    ...
    print("lukis can generate images through your Grok (xAI) subscription — free, but "
...
def cmd_doctor(args):
    ...
    if _config_is_stale(cfg):
        ...
        lines.append("backend: grok-bot — transport: agent-side native Grok Bot "
                     "image tool (not `lukis.py generate`; no CLI/OpenRouter key "
                     "required)")
    elif backend == "codex":
        ready = codex_ok
        lines.append("backend: codex — transport: `codex exec` (your Codex "
                     "subscription, gpt-image-2; lukis stores no token)"
                     if ready else
        ...
        lines.append("backend: grok — transport: `grok -p` (your Grok "
                     "subscription; lukis stores no token)"
                     if ready else
        ...
        lines.append("note: Grok Bot agents can use Grok Bot's built-in Grok "
                     ...
                     "for that path.")
    # Hermes caveat: the path above is lukis's default; a managed runtime may
...
def cmd_newrun(args):
    """Make + print a fresh run dir for a batch: $LUKIS_TMP (or /tmp/lukis) / <runid>."""
...
def fetch(url, optional=False):
    req = urllib.request.Request(url, headers={"User-Agent": "lukis-skill"})
    try:
...
def build_gallery_html(recs, embed, base, title=None, request=None):
    ...
    heading = _html.escape(title or "Lukis gallery")
    req_html = ""
    if request:
        if len(request) > 280:
...
def main():
    ap = argparse.ArgumentParser(description="Lukis editorial illustration engine.")
    ...
    nr = sub.add_parser("newrun", help="make + print a fresh batch dir (/tmp/lukis/<runid>)")
    ...
    pi = pksub.add_parser("install", help="install a pack, or --all packs, into ~/.config/lukis/characters/")
