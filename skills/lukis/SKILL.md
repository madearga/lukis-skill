...
description: >-
  ...
  (pose-only compositing asset, no scene or text) — in one of eighteen bundled
  looks (seventeen print, plus a photoreal toy-brick set). Also handles
  ...
  the skill is directly invoked or "lukis" is requested; never on generic
...
author: I Made Arga Swarsa
license: MIT
metadata:
  hermes:
    tags: [illustration, sablon, image-generation, editorial, mascot, codex, grok, openrouter]
    category: creative
    requires_toolsets: [terminal]
  openclaw:
    emoji: "🎨"
    homepage: https://github.com/madearga/lukis-skill
    os: [macos, linux]
    requires:
      bins: [python3]
---
...
per pack, chosen from the bundled look library (sablon — flat hand-pulled
screenprint, thick stencil line, bare-paper seams between ink layers — plus
riso, blueprint, woodcut, pixel, clay, manila, chalk, phosphor, enamel,
gouache, felt, diorama, sketchbook, bricks, fizz, bloom, and snes) or a custom style file. The default mascot is
**Ciko**, a deadpan house gecko in sablon. Palettes come
...
| **Social-ready art for X posts / article body images** | 16:9 (or 1:1 when square is explicitly useful), bold `pasar`, watermark with the `x` handle if configured or asked. |
| **X Article banner / hero image** | Use the unique banner format: **1536 × 640 px** when the user asks for an X Article hero/banner. Prompt and render through the normal `lukis.py generate` image pipeline, with normal, undistorted character/object proportions and crop-safe breathing room. Do not satisfy this by manually compositing or rebuilding crops from another image unless the user explicitly asks for post-processing. |
...
| **Their own mascot** — "make me a character", "use our mascot", "replace Ciko" | The character builder: read `references/character-builder.md` in full and follow it end to end. |
...
The engine (`scripts/lukis.py`, stdlib Python, no installs) renders through one
...
`lukis.py generate`, when no user config explicitly selects an engine backend.

**Running the engine — set `$SKILL_DIR` inline in each block.** Every engine
command below is `python3 "$SKILL_DIR/scripts/lukis.py" …`. Set `SKILL_DIR` to the
absolute path of the directory this `SKILL.md` was loaded from (it contains
`scripts/lukis.py` and `assets/`) **in the same command block that uses it** — shell
...
installed `scripts/lukis.py` and use its parent; if neither resolves, stop rather
than guessing the working directory. The engine self-locates its own bundled
assets, so `$SKILL_DIR` only has to be right enough to launch `lukis.py` and to
...
applies, so the path collapses to `/scripts/lukis.py`). Put **no comment on an
...
`doctor` (Workflow step 0) fail loudly (`can't open file …/scripts/lukis.py`) — the
...
- **Codex backend (free for Codex subscribers).** When the host has a usable
  **Codex CLI** — installed, `codex login`-ed, with the `image_generation`
  feature — lukis can generate through the user's Codex subscription at no
  per-image charge (it draws on their Codex quota). No API key, no token: lukis
...
- **Grok CLI backend (free for Grok/xAI subscribers).** When the host has a usable
  **Grok CLI** — installed and `grok login`-ed — lukis can generate through the
...
- **Grok Bot native transport (agent-side, free for Grok Bot users).** When
  **you are Grok Bot** — specifically Cursor's Grok Bot / the Grok desktop
  assistant with the built-in Grok image tool — build the lukis prompt and call
  ...
  "host image API" rule and not an `lukis.py --backend` value.
...
- **Bootstrap (user runs it):** `python3 "$SKILL_DIR/scripts/lukis.py" init` —
  prompts for the key at a hidden prompt (never echoed) and writes the
  YAML config `${XDG_CONFIG_HOME:-~/.config}/lukis/config.yaml` (mode 600). It
...
- `references/styles/sablon.md` — sablon, the house default look: the hand-pulled screenprint technique, line language, paper/ink, hard do/don'ts.
- `references/styles/<name>.md` — the rest of the look library (`riso`, `blueprint`, `woodcut`, `pixel`, `clay`, `manila`, `chalk`, `phosphor`, `enamel`, `gouache`, `felt`, `diorama`, `sketchbook`, `bricks`, `fizz`, `bloom`, `snes`), consumed by character packs. Read the active character's style file in full before generating.
- `references/character.md` — the character rules (the load-bearing test, anti-complexity guardrails, value-follows-palette, the **interaction model** — declared per pack or derived conservatively from the locked design and reference sheet), the default character **Ciko**, and the custom-pack format. Read before any character work.
...
python3 "$SKILL_DIR/scripts/lukis.py" doctor
```

Run the `lukis.py` call standalone — never chained with `&&` — so the displayed exit code is
...
```bash
SKILL_DIR="<path to this skill>";
python3 "$SKILL_DIR/scripts/lukis.py" init --backend grok-bot --no-key
```
...
Skip `lukis.py init` for OpenRouter unless the user explicitly wants OpenRouter
or another engine backend default, and skip `lukis.py generate` unless the user
...
going to use `lukis.py generate`, if `doctor` reports `backend: NEEDS CHOICE`
...
"lukis now has image backends/transports — which would you like?" with four
...
`python3 "$SKILL_DIR/scripts/lukis.py" init --backend <codex|grok|grok-bot|openrouter> --no-key`,
...
profile's home (e.g. `…/profiles/<name>/home/.config/lukis/…`), so a key
...
`python3 "$SKILL_DIR/scripts/lukis.py" init` themselves — do not enter the
...
`python3 <resolved absolute $SKILL_DIR>/scripts/lukis.py init` (hidden
prompt), or open `~/.config/lukis/config.yaml` and fill in the `apiKey:`
...
`python3 "$SKILL_DIR/scripts/lukis.py" packs list` flags stale installs
...
in `references/surprise.md` (named pack, or random among installed + Ciko;
never `defaultCharacter`). Continue at Step 3+.

Installed packs live under `${XDG_CONFIG_HOME:-~/.config}/lukis/characters/`
...
1. **Explicit request** — "use <pack name>", "as <name>": that pack (or the
   shipped default when asked for by name, `ciko`). When the word matches no
...
3. **Shipped default** — **Ciko** (spec in `references/character.md`, model
...
(`references/styles/<name>.md`) or a custom one at
`${XDG_CONFIG_HOME:-~/.config}/lukis/styles/<name>.md`; absent line = sablon.
Ciko is sablon. For any non-riso style, read its file in full: it supplies the
...
applies, do **not** run `lukis.py generate`. Use the same full prompt recipe,
...
sheet as a reference image (`assets/character-reference.webp` for Ciko, or the
...
python3 "$SKILL_DIR/scripts/lukis.py" generate --prompt-file /tmp/shot-01.txt --ref "$REF" --aspect 16:9 --out "assets/<slug>-illustrations/01-topic.png"
```

For engine renders, `lukis.py generate` prints a **JSON line per image**
...
substitute for an lukis render. Check `.width/.height`, and only do final
...
`newrun` prints a fresh run dir (`/tmp/lukis/<runid>`) into `RUN`. Record the user's
...
```bash
SKILL_DIR="<path to this skill>";
RUN=$(python3 "$SKILL_DIR/scripts/lukis.py" newrun);
printf '%s' "<the verbatim request>" > "$RUN/request.txt"
# (a) VARIATIONS — same prompt+model, pick-the-best:
python3 .../lukis.py generate --prompt-file p.txt --ref <ref> --count 4 --label "draft→ship" --out "$RUN/v.png"
...
for m in <model-id-1> <model-id-2>; do
  python3 .../lukis.py generate --prompt-file p.txt --ref <ref> --model "$m" --label "$m" --out "$RUN/$(basename $m).png"; done
# (c) CONCEPT VARIATIONS — different prompts (different stagings) for one idea:
python3 .../lukis.py generate --prompt-file staging-A.txt --ref <ref> --label "as a funnel" --out "$RUN/a.png"
python3 .../lukis.py generate --prompt-file staging-B.txt --ref <ref> --label "as a crossing" --out "$RUN/b.png"

python3 "$SKILL_DIR/scripts/lukis.py" gallery "$RUN" --title "<the piece or request>" --open
...
- **Chat sessions** (the user is on a messaging surface — Hermes over
  ...
  **original file**. Platform photo delivery recompresses images — exactly
  what destroys print texture — halftone grain, stencil edges, bare-paper seams, and fine
