...
class PullRequestCommitValidationTests(unittest.TestCase):
    ...
    def test_merge_strategy_validates_release_please_child_commits(self):
        ...
        self.assertEqual(
            self.validator.validate(commits, ["skills/lukis/references/styles/snes.md"]),
            [],
        )

        errors = self.validator.validate(
            [self.commit("Add SNES look")],
            ["skills/lukis/references/styles/snes.md"],
        )
    ...
    def test_installed_skill_fix_and_feat_commits_are_valid(self):
        for subject in (
            ...
            "feat(lukis)!: replace the rendering contract",
        ):
            with self.subTest(subject=subject):
                self.assertEqual(
                    self.validator.validate(
                        [self.commit(subject)], ["skills/lukis/SKILL.md"]
                    ),
                    [],
                )
    ...
    def test_installed_skill_with_only_ci_or_docs_is_rejected(self):
        ...
        errors = self.validator.validate(commits, ["skills/lukis/SKILL.md"])
    ...
    def test_trusted_generated_release_metadata_is_valid(self):
        errors = self.validator.validate(
            ...
            ["skills/lukis/SKILL.md", "version.txt"],
