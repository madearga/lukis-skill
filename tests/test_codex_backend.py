...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / "skills" / "lukis" / "scripts" / "lukis.py"


def load_lukis_module():
    spec = importlib.util.spec_from_file_location("lukis_script_under_test", SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load lukis script from {SCRIPT_PATH}")
...
def write_png_artifact(path):
    """Write enough PNG structure for lukis's artifact validation tests."""
    path.write_bytes(
        b"\x89PNG\r\n\x1a\n"
...
class CodexBackendTests(unittest.TestCase):
    def setUp(self):
        self.lukis = load_lukis_module()
        self.lukis.codex_available = lambda: True
        self.original_run = self.lukis.subprocess.run

    def tearDown(self):
        self.lukis.subprocess.run = self.original_run
    ...
    def test_codex_exec_passes_refs_and_does_not_enable_removed_feature(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.lukis.subprocess.run = fake_run
            produced, meta = self.lukis.codex_exec_generate(
        ...
        # image_generation); lukis must no longer enable it.
        ...
        self.assertEqual(captured["timeout"], self.lukis.CODEX_EXEC_TIMEOUT)
    ...
    def test_codex_manifest_records_the_actual_render_prompt(self):
        ...
        self.lukis.codex_exec_generate = fake_generate
        with tempfile.TemporaryDirectory() as td:
            rec = self.lukis._render_one(
    ...
    def test_nonzero_exit_with_fresh_nested_generated_artifact_succeeds(self):
        with tempfile.TemporaryDirectory() as codex_home, tempfile.TemporaryDirectory() as td:
            artifact = generated_session(
                codex_home, self.lukis.CODEX_GENERATED_SUBDIR
            ) / "render.png"
    ...
    def test_timeout_with_fresh_nested_generated_artifact_succeeds(self):
        with tempfile.TemporaryDirectory() as codex_home, tempfile.TemporaryDirectory() as td:
            ...
            def fake_run(cmd, input, capture_output, text, timeout):
                write_png_artifact(artifact)
                raise self.lukis.subprocess.TimeoutExpired(cmd, timeout)
    ...
    def test_thread_id_parser_accepts_text_and_timeout_bytes(self):
        ...
        for output in (thread_started_json(thread_id),
            ...
            with self.subTest(output_type=type(output).__name__):
                self.assertEqual(self.lukis._codex_thread_id(output), thread_id)
        self.assertIsNone(self.lukis._codex_thread_id(None))
    ...
    def test_thread_id_parser_rejects_unsafe_path_components(self):
        for thread_id in ("", ".", "..", "../foreign", "foreign/session",
            ...
            with self.subTest(thread_id=repr(thread_id)):
                self.assertIsNone(
                    self.lukis._codex_thread_id(thread_started_json(thread_id)))

    def test_thread_event_prefers_own_artifact_over_newer_foreign_session(self):
        with tempfile.TemporaryDirectory() as codex_home, tempfile.TemporaryDirectory() as td:
            generated = Path(codex_home) / self.lukis.CODEX_GENERATED_SUBDIR
    ...
    def test_detect_codex_accepts_image_generation_alone(self):
        self.lukis.shutil.which = lambda name: "/usr/local/bin/codex" if name == "codex" else None
        ...
        self.lukis._codex_run = fake_codex_run
        self.assertTrue(self.lukis._detect_codex())
    ...
    def test_detect_codex_rejects_when_image_generation_absent(self):
        ...
        self.lukis._codex_run = fake_codex_run
        self.assertFalse(self.lukis._detect_codex())
    ...
    def test_generic_codex_error_includes_redacted_combined_output(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            self.lukis.subprocess.run = fake_run
            with self.assertRaises(self.lukis.BackendUnavailable) as ctx:
                self.lukis.codex_exec_generate("prompt", [], Path(td) / "out.png")
    ...
    def test_nonzero_exit_with_invalid_fresh_nested_artifact_fails(self):
        with tempfile.TemporaryDirectory() as codex_home, tempfile.TemporaryDirectory() as td:
            ...
            self.lukis.subprocess.run = fake_run
            with mock.patch.dict(os.environ, {"CODEX_HOME": codex_home}):
                with self.assertRaises(self.lukis.BackendUnavailable):
                    self.lukis.codex_exec_generate(
                        "draw the mascot", [], Path(td) / "requested.png"
                    )
    ...
    def test_count_batch_prior_artifact_not_reused_after_timeout(self):
        ...
        def fake_run(cmd, input, capture_output, text, timeout):
            raise self.lukis.subprocess.TimeoutExpired(
                cmd, timeout,
    ...
    def test_count_batch_fresh_artifact_still_detected_alongside_prior(self):
        ...
        with tempfile.TemporaryDirectory() as codex_home, tempfile.TemporaryDirectory() as td:
            ...
            fresh_session = (
                Path(codex_home) / self.lukis.CODEX_GENERATED_SUBDIR / "other-session"
            )
...
class PaidFallbackTests(unittest.TestCase):
    def setUp(self):
        ...
        def fail_codex(*args, **kwargs):
            raise self.lukis.BackendUnavailable("wrapper failed")

        self.lukis.codex_exec_generate = fail_codex
    ...
    def test_default_cli_failure_never_calls_openrouter_with_configured_key(self):
        ...
        self.lukis._openrouter_record = unexpected_openrouter
        with tempfile.TemporaryDirectory() as td:
            with self.assertRaises(SystemExit) as ctx:
                self.lukis._render_one(
    ...
    def test_explicit_paid_fallback_records_openrouter_backend(self):
        ...
        self.lukis._openrouter_record = fake_openrouter
        codex_prompt = self.lukis.cutout_prompt_for_backend(
            "prompt", "codex", self.lukis.CHROMA_GREEN
        )
        with tempfile.TemporaryDirectory() as td:
            rec = self.lukis._render_one(
                "codex", {"apiKey": "configured"}, codex_prompt, "model",
                ["ref.png"], False, Path(td) / "out.png", cutout=True,
                chroma_key=self.lukis.CHROMA_GREEN, allow_paid_fallback=True
            )
    ...
    def test_direct_openrouter_does_not_require_paid_fallback_flag(self):
        self.lukis._openrouter_record = lambda *args, **kwargs: {"backend": "openrouter"}
        with tempfile.TemporaryDirectory() as td:
            rec = self.lukis._render_one(
