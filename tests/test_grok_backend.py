...
class GrokExecTests(unittest.TestCase):
    def setUp(self):
        ...
        self.lukis.grok_available = lambda: True
    ...
    def test_grok_exec_sandboxes_and_uses_image_edit_with_refs(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            produced, meta = self.lukis.grok_exec_generate(
        ...
        self.assertEqual(captured["timeout"], self.lukis.GROK_EXEC_TIMEOUT)
    ...
    def test_grok_exec_uses_image_gen_without_refs(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.lukis.subprocess.run = fake_run
            self.lukis.grok_exec_generate("ref-less bootstrap", [], out_path)
    ...
    def test_grok_exec_falls_back_to_session_cache_when_out_missing(self):
        ...
        with tempfile.TemporaryDirectory() as grok_home, tempfile.TemporaryDirectory() as td:
            ...
            self.lukis.grok_home = lambda: Path(grok_home)

            def fake_run(cmd, capture_output, text, timeout):
                return FakeCompletedProcess(returncode=0)  # writes nothing to --out

            self.lukis.subprocess.run = fake_run
            produced, _ = self.lukis.grok_exec_generate("prompt", [], Path(td) / "out.png")
    ...
    def test_grok_error_is_redacted_and_backend_unavailable(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(self.lukis.BackendUnavailable) as ctx:
                self.lukis.grok_exec_generate("prompt", [], Path(td) / "out.png")
...
class GrokDetectionTests(unittest.TestCase):
    ...
    def test_available_requires_binary_and_auth_file(self):
        with tempfile.TemporaryDirectory() as home:
            self.lukis.grok_home = lambda: Path(home)
            self.lukis.shutil.which = lambda name: "/usr/local/bin/grok" if name == "grok" else None

            self.lukis._GROK_AVAILABLE = None
            self.assertFalse(self.lukis.grok_available())  # no auth.json yet

            (Path(home) / "auth.json").write_text("{}")
            self.lukis._GROK_AVAILABLE = None
            self.assertTrue(self.lukis.grok_available())

            self.lukis.shutil.which = lambda name: None
            self.lukis._GROK_AVAILABLE = None
            self.assertFalse(self.lukis.grok_available())  # binary gone
...
class CutoutModelRoutingTests(unittest.TestCase):
    ...
    def test_cutout_always_resolves_gpt_image_regardless_of_backend(self):
        ...
        for backend in ("codex", "grok", "openrouter"):
            self.assertEqual(
                self.lukis.resolve_generate_model(cfg, None, backend, cutout=True),
                self.lukis.CUTOUT_OPENROUTER_MODEL,
                backend,
            )
        # Explicit --model still wins for cutouts.
        self.assertEqual(
            self.lukis.resolve_generate_model(cfg, "user/model", "grok", cutout=True),
            "user/model",
        )
        # Editorial keeps config/default resolution.
        self.assertEqual(
            self.lukis.resolve_generate_model(cfg, None, "grok", cutout=False),
...
class RenderRefResolutionTests(unittest.TestCase):
    ...
    def _with_default_char(self, cfgdir):
        ...
        self.lukis.config_dir = lambda: Path(cfgdir)
        return char_ref
    ...
    def test_direct_openrouter_gets_default_character_ref(self):
        with tempfile.TemporaryDirectory() as cfgdir:
            ...
            captured = {}
            self.lukis._openrouter_record = (
                lambda cfg, prompt, model, refs, *a, **k: captured.update(refs=refs) or {}
            )
            cfg = {"apiKey": "k", "defaultCharacter": "bot"}
            self.lukis._render_one("openrouter", cfg, "p", "m", [], False, Path(cfgdir) / "o.png")
    ...
    def test_cli_fallback_to_openrouter_keeps_default_character_ref(self):
        with tempfile.TemporaryDirectory() as cfgdir:
            ...
            def boom(*a, **k):
                raise self.lukis.BackendUnavailable("down")

            self.lukis.grok_exec_generate = boom
            captured = {}
            self.lukis._openrouter_record = (
                lambda cfg, prompt, model, refs, *a, **k: captured.update(refs=refs) or {}
            )
