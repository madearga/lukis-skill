...
class CutoutChromaTests(unittest.TestCase):
    ...
    def test_codex_cutout_prompt_defaults_to_native_alpha(self):
        prompt = self.lukis.cutout_prompt_for_backend(
            "Ciko carrying a pencil",
            "codex",
            self.lukis.CHROMA_MAGENTA,
        )
    ...
    def test_generate_applies_native_contract_after_codex_routing(self):
        ...
        self.lukis.load_config = lambda: {"configVersion": self.lukis.CONFIG_VERSION}
        self.lukis.resolve_backend = lambda cfg, requested: "codex"
        self.lukis._render_one = fake_render
        with tempfile.TemporaryDirectory() as td:
            args = types.SimpleNamespace(
                prompt="Ciko carrying a pencil",
                prompt_file=None,
                backend="codex",
                model=None,
                cutout=True,
    ...
    def test_grok_cutout_redirect_applies_codex_native_contract(self):
        ...
        self.lukis.load_config = lambda: {"configVersion": self.lukis.CONFIG_VERSION}
        self.lukis.resolve_backend = lambda cfg, requested: "grok"
        self.lukis.codex_available = lambda: True
        self.lukis._render_one = fake_render
        with tempfile.TemporaryDirectory() as td:
            args = types.SimpleNamespace(
                prompt="Ciko carrying a pencil", prompt_file=None, backend="grok",
    ...
    def test_interior_pink_accent_does_not_discard_chroma_cutout(self):
        ...
        keyed = self.lukis.chroma_key_to_png(source, key=self.lukis.CHROMA_MAGENTA)
        self.assertIsNotNone(keyed)
        analysis = self.lukis.analyze_cutout_alpha(keyed)
        self.assertTrue(analysis["clean_alpha"])
        self.assertEqual(analysis["accent_halo"], 0)

        with tempfile.TemporaryDirectory() as td:
            out, _, _, meta = self.lukis.place_cutout_image(
                source, Path(td) / "cutout.png", chroma_key=self.lukis.CHROMA_MAGENTA
            )
            parsed = self.lukis._parse_png_rgb_or_rgba(out.read_bytes())
    ...
    def test_chroma_fallback_preserves_existing_transparent_non_key_pixels(self):
        ...
        self.assertFalse(self.lukis.analyze_cutout_alpha(source)["clean_alpha"])
    ...
    def test_adjacent_interior_soft_pixels_do_not_self_trigger_fringe(self):
        ...
        analysis = self.lukis.analyze_cutout_alpha(source)
        ...
        self.assertIsNone(self.lukis._cutout_quality_note(analysis))
    ...
    def test_pink_ring_behind_wide_soft_matte_warns_without_discarding_cutout(self):
        ...
        self.assertTrue(analysis["clean_alpha"])
        self.assertGreaterEqual(analysis["accent_halo"], self.lukis.CUTOUT_FRINGE_WARN)
